...
for sz in 4096 131072 4194304 ; do
  ...
  pb "./rados-obj-perf.sh --obj-size $sz --obj-count $count --request-type write --threads $threads --adjust-think-time true" obj.write.sz.$sz.cnt.$count
  pb "./rados-obj-perf.sh --obj-size $sz --obj-count $count --request-type read --threads $threads --adjust-think-time true" obj.read.sz.$sz.cnt.$count
done
#for kcount in 64 1024 ; do
for kcount in 16 1024 65536 ; do
  (( count = $kcount * 1024 ))
  ...
  cmd="$cmd --threads $threads --adjust-think-time true "
